...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`tree-sitter-bash`](https://crates.io/crates/tree-sitter-bash).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `tree-sitter-bash` crate will be cloned into `tmp/` (gitignored) when porting begins.
...
Original work copyrighted by the upstream `tree-sitter-bash` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
